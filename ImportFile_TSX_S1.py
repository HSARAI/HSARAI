...
def ImportData():
    ...
    if  Nclasses+1>4 :  
        ...
        # Loading train images
        images_path = "/industrialfacilities/"
    ...
    if  Nclasses+1>7 :  
        ...
        for img in images:
            ...
            image1 = image#[:,:,1]#-127            #image1 = cv2.resize(image1, (ww,hh), interpolation = cv2.INTER_AREA)
    ...
    if  Nclasses==8 : 
        ...
        np.save('test_images_training'+str(Training_ratio) +'_lookstep'+str(Look_step),test_images)
        np.save('test_labels_training'+str(Training_ratio) +'_lookstep'+str(Look_step),test_labels)
        #np.save('values_test_training'+str(Training_ratio) +'_lookstep'+str(Look_step),values_test)
        np.save('test_labels_H_training'+str(Training_ratio) +'_lookstep'+str(Look_step),test_labels_H)
        
        np.save('train_images_training'+str(Training_ratio) +'_lookstep'+str(Look_step),train_images)
        np.save('train_labels_training'+str(Training_ratio) +'_lookstep'+str(Look_step),train_labels)
        np.save('train_labels_H_training'+str(Training_ratio) +'_lookstep'+str(Look_step),train_labels_H)

        #np.save('values_train_training'+str(Training_ratio) +'_lookstep'+str(Look_step),values_train)
